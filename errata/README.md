...
## 페이지 : 378
### 연습문제 6.12 : 문제가 명확하지 않음(차후에 더욱더 명확하게 하겠습니다)
문제가 명확하지 않아 다음과 같이 수정합니다.
<pre>
6.11 다음과 같은 리스트에는 원소를 가지는 항목과 원소를 가지지 않는 빈 튜플, 빈 문자열, 빈 리스트가 존재한다. 이 리스트에서 빈 튜플과 빈 문자열, 빈 리스트만을 제거한 결과를 출력하는 코드를 작성하라. 단 ((),) 튜플은 빈 튜플이 하나 있는 튜플로 간주하여 제거하지 않는다.
[실행결과]
